...
def _path(opt):
    ...
    data_dir = os.path.join(opt['datapath'], 'multiwoz_value', 'MULTIWOZ2.1')
...
class MultiWozDSTTeacher(FixedDialogTeacher):
    ...
    def __init__(self, opt, shared=None):
        ...
        self.id = 'multiwoz_value'

        # # # reading args
        self.decode_all = opt.get('decode_all', False)
        self.just_test = opt.get('just_test', False)
        self.generated_type_result_path = opt.get('generated_type_result_path', False)
    ...
    def _setup_data(self, data_path, data_dir):
        ...
        if self.decode_all:
            ...
            # import pdb
            # pdb.set_trace()
            if self.generated_type_result_path is not None:
                modify = Modify(None, data_dir)
                self.messages = modify.replace_domtype(self.messages, self.generated_type_result_path)
    ...
    def get(self, episode_idx, entry_idx=0):
        ...
        entry = self.messages[episode_idx]['context']  + \
                " <bs> " + self.messages[episode_idx]['slots_domtype'] + " </bs>"
        episode_done = True
        action = {
            'id': self.id,
            'text': entry,
