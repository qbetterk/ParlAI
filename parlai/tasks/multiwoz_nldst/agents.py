...
def _path(opt):
    ...
    data_dir = os.path.join(opt['datapath'], 'multiwoz_nldst', 'MULTIWOZ2.1')
    # data_dir = os.path.join('/checkpoint/kunqian/multiwoz/data/MultiWOZ_2.1/')
    reformat_data_name = 'data_reformat_trade_turn.json'
    data_path = os.path.join(data_dir, reformat_data_name)
    ...
    if not os.path.exists(data_path):
        reformat_parlai(data_dir, reformat_data_name)
...
class MultiWozDSTTeacher(FixedDialogTeacher):
    ...
    def _setup_data(self, data_path, jsons_path):
        ...
        if self.decode_all:
            ...
            random.seed(0)
            self.messages = random.sample(list(valid_data.values()), k=2000)
        else:
