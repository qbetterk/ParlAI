...
def _path(opt):
    ...
    data_dir = os.path.join(opt['datapath'], 'multiwoz_value_alltype_seq')
...
class MultiWozDSTTeacher(FixedDialogTeacher):
    ...
    def __init__(self, opt, shared=None):
        ...
        self.id = 'multiwoz_value_alltype_seq'
    ...
    def get(self, episode_idx, entry_idx=0):
        # if entry_idx > len(self.messages[episode_idx]['slots_dom_type']) - 1:
        #     import pdb
        #     pdb.set_trace()
        if self.messages[episode_idx]['slots_dom_type'] != []:
            dom_type = self.messages[episode_idx]['slots_dom_type'][entry_idx]
            label = self.messages[episode_idx]['slots_inf'][entry_idx]
        else:
            dom_type = ""
            label = ""
        ...
        if entry_idx == 0:
            ...
            entry = "<bs> " + dom_type + " </bs>"

        episode_done = (entry_idx >= len(self.messages[episode_idx]['slots_dom_type']) - 1)
